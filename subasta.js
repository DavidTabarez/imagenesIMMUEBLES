...
function productosAprobados() {
  ...
  return productos.filter(p => estados[p.id] === "aprobado");
}
  ...
  if (aprobados.length === 0) {
    contenedor.innerHTML = `
      <div style="text-align:center;padding:80px 20px;color:#94a3b8">
        ...
        <h2 style="font-size:20px;margin-bottom:8px;color:#64748b">No hay productos disponibles aún</h2>
        <p style="font-size:14px">El administrador está revisando los productos. Vuelve pronto.</p>
      </div>`;
    return;
  }
...
function crearCard(p) {
  ...
  card.innerHTML = `
    ...
    <div class="content">
      ...
      <button class="btn-ver" onclick="abrirPanel(${p.id})">Ver producto</button>
    </div>
  `;
  return card;
}
